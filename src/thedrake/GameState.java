...
public class GameState implements JSONSerializable {
    ...
    public boolean canPlaceFromStack(TilePos target) {
        ...
        BoardPos boardTarget = (BoardPos) target;
        BoardTroops troopsOnTurn = armyOnTurn().boardTroops();

        if (!troopsOnTurn.isLeaderPlaced()) {
            if (sideOnTurn() == PlayingSide.BLUE) {
                return boardTarget.j() == 0;
            } else {
                return boardTarget.j() == board.dimension() - 1;
            }
        }

        if (troopsOnTurn.isPlacingGuards()) {
            return boardTarget.isNextTo(troopsOnTurn.leaderPosition());
        }

        for (BoardPos troopPos : troopsOnTurn.troopPositions()) {
            if (troopPos.isNextTo(boardTarget)) {
                return true;
            }
        }
        return false;
